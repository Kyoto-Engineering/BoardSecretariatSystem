...
namespace BoardSecretariatSystem
{
    ...
    public partial class AgendaEntryUI : Form
    {
        ...
        public string userId,boardId;

        public int company_id, agenda_id,meetingId;
        ...
        private void agendaSaveButton_Click(object sender, EventArgs e)
        {
            ...
            else if (string.IsNullOrEmpty(boardNameComboBox.Text))
            {
                ...
                try
                {
                    con = new SqlConnection(cs.DBConn);
                    con.Open();
                    string query1 = "insert into t_agenda (AgendaTopics,BoardId,MeetingId,UserId,DateTime) values (@d1,@d2,@d3,@d4,@d5)" + "SELECT CONVERT(int, SCOPE_IDENTITY())";
                    cmd = new SqlCommand(query1, con);
                    cmd.Parameters.AddWithValue("@d1", topicsRichTextBox.Text);
                    cmd.Parameters.AddWithValue("@d2", boardId);
                    cmd.Parameters.AddWithValue("@d3", meetingId);
